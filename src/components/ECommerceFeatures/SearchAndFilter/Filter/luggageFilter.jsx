...
// Component to display Luggage filtered products
const LuggageFilter = () => {
  ...
  useEffect(() => {
    ...
    // Fetching the list of Luggage from the backend API
    axios
      .get(
        "https://mern-ecommerce-website-2tn3.onrender.com/api/luggages"
      )
      .then((res) => {
  ...
  const handleRemoveProduct = async (id) => {
    try {
      await axios.delete(
        `https://mern-ecommerce-website-2tn3.onrender.com/api/luggages/${id}`
      );
  ...
  return (
    <>
      <Header />
      <main className="flex">
        <FilterTitles luggage={"font-bold"} />
